...
// Централизиран списък с категории (може да се изнесе и в db.php или config файл)
$categories = [
    "Sensors & Boards", 
    "Motors & Drivers", 
    "Tools & Equipment", 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO items (name, category, location, quantity, min_quantity) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([$name, $category, $location, $quantity, $min_quantity]);
...
<head>
    ...
    <title>Add New Item - EQE Inventory</title>
...
<body>
    ...
    <div class="container" style="max-width: 650px;">
        <div class="form-card p-4 p-md-5">
            <h3 class="fw-extrabold mb-4 pb-3 border-bottom text-primary" style="color: var(--ottobock-blue); font-weight: 900;">
            ...
            <form method="POST">
                <div class="mb-4">
                    ...
                    <input type="text" name="name" class="form-control form-control-lg fs-6 rounded-3" placeholder="e.g. Proximity Sensor OMRON" required>
                </div>
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="location" class="form-control form-control-lg fs-6 rounded-3" placeholder="e.g. Rack A, Shelf 2">
                    </div>
                </div>
                ...
                <div class="row g-3 mb-5">
                    <div class="col-md-6">
                        ...
                        <input type="number" name="quantity" class="form-control form-control-lg fs-6 rounded-3" value="1" min="0">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label fw-bold small text-uppercase">Min. Reorder Limit</label>
                        <input type="number" name="min_quantity" class="form-control form-control-lg fs-6 rounded-3" value="2" min="0">
                    </div>
                </div>

                <div class="d-flex justify-content-between align-items-center pt-3 border-top">
                    <a href="index.php" class="btn btn-light rounded-pill px-4 fw-bold text-secondary">Cancel</a>
                    <button type="submit" class="btn btn-ottobock px-5">Save Item</button>
                </div>
            </form>
        </div>
    </div>
